...
module.exports = {
    ...
    doMP4: function(sources, dest){
        var defer = Q.defer();
        //compiled version
        exec("/usr/local/bin/ffmpeg -r 10 -y -i "+sources+" -vcodec libx264 -preset slow -crf 24 -pix_fmt yuv420p "+dest+"", function(error, stdout, stderr){
    ...
    doOGG: function(sources, dest){
        var defer = Q.defer();
        //ubuntu version
        exec("/usr/local/bin/ffmpeg -r 10 -y -i "+sources+" -qscale 15 "+dest, function(error, stdout, stderr){
